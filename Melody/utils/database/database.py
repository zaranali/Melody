...
from Melody import userbot
from Melody.core.mongo import mongodb
from Melody.core.redis import redis_client

"""
DATABASE ABRACTION LAYER
This module manages both MongoDB persistent storage (for settings/auth) 
and high-speed in-memory/Redis caching (for active calls/loops).
"""
...
async def get_playtype(chat_id: int) -> str:
    if redis_client:
        mode = redis_client.get(f"playtype_{chat_id}")
        if mode:
            return mode
    ...
    if not mode:
        ...
        if not mode:
            ...
            if redis_client:
                redis_client.set(f"playtype_{chat_id}", "Everyone")
            return "Everyone"
        playtype[chat_id] = mode["mode"]
        if redis_client:
            redis_client.set(f"playtype_{chat_id}", mode["mode"])
        return mode["mode"]
    return mode
...
async def set_playtype(chat_id: int, mode: str):
    ...
    if redis_client:
        redis_client.set(f"playtype_{chat_id}", mode)
...
async def get_playmode(chat_id: int) -> str:
    if redis_client:
        mode = redis_client.get(f"playmode_{chat_id}")
        if mode:
            return mode
    ...
    if not mode:
        ...
        if not mode:
            ...
            if redis_client:
                redis_client.set(f"playmode_{chat_id}", "Direct")
            return "Direct"
        playmode[chat_id] = mode["mode"]
        if redis_client:
            redis_client.set(f"playmode_{chat_id}", mode["mode"])
        return mode["mode"]
    return mode
...
async def set_playmode(chat_id: int, mode: str):
    ...
    if redis_client:
        redis_client.set(f"playmode_{chat_id}", mode)
...
async def get_lang(chat_id: int) -> str:
    if redis_client:
        lang = redis_client.get(f"lang_{chat_id}")
        if lang:
            return lang
    ...
    if not mode:
        ...
        if not lang:
            langm[chat_id] = "en"
            if redis_client:
                redis_client.set(f"lang_{chat_id}", "en")
            return "en"
        langm[chat_id] = lang["lang"]
        if redis_client:
            redis_client.set(f"lang_{chat_id}", lang["lang"])
        return lang["lang"]
    return mode
...
async def set_lang(chat_id: int, lang: str):
    langm[chat_id] = lang
    if redis_client:
        redis_client.set(f"lang_{chat_id}", lang)
