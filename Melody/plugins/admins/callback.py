...
from Melody import YouTube, app
from Melody.core.call import MelodyCall
from Melody.misc import SUDOERS, db
...
from Melody.utils.inline import close_markup, stream_markup, stream_markup_timer
from Melody.utils.inline.help import help_pannel_page1, help_pannel_page2, help_pannel_page3
...
async def show_help_page1(client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_caption(
        caption=_["help_1"].format(f"https://t.me/{SUPPORT_GROUP}"),
...
from Melody.utils.inline.start import about_panel
from strings import get_string
from config import BANNED_USERS

from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardMarkup
from Melody.utils.inline.start import owner_panel  # Import owner_panel function
from strings import get_string
from config import BANNED_USERS
...
async def about_cb(client, callback_query):
    try:
        ...
        await callback_query.edit_message_caption(
            caption=_["about_1"].format(app.mention),
            reply_markup=InlineKeyboardMarkup(about_panel(_))
        )
    except Exception as e:
        log.error(f"About callback error: {e}")
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command in ["Pause", "Resume", "Toggle"]:
        if command == "Pause":
            ...
            await MelodyCall.pause_stream(chat_id)
            await CallbackQuery.message.reply_text(
                _["admin_2"].format(mention), reply_markup=close_markup(_)
            )
            try:
                await CallbackQuery.edit_message_reply_markup(
                    reply_markup=InlineKeyboardMarkup(stream_markup(_, chat_id, is_playing=False))
                )
            except:
                pass
            ...
            await MelodyCall.resume_stream(chat_id)
            await CallbackQuery.message.reply_text(
                _["admin_4"].format(mention), reply_markup=close_markup(_)
            )
            try:
                await CallbackQuery.edit_message_reply_markup(
                    reply_markup=InlineKeyboardMarkup(stream_markup(_, chat_id, is_playing=True))
                )
            except:
                pass
        ...
        return await CallbackQuery.answer("No previous track in history!", show_alert=True)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await MelodyCall.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await MelodyCall.skip_stream(chat_id, link, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])

            button = stream_markup(_, chat_id, is_playing=True)
            ...
            try:
                await MelodyCall.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await MelodyCall.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            ...
            try:
                await MelodyCall.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
