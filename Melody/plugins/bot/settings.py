...
from Melody.utils.decorators.admins import ActualAdminCB
from Melody.utils.decorators.language import language, languageCB
from Melody.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from Melody.utils.inline.start import private_panel
from config import BANNED_USERS, OWNER_ID, DEV_ID
...
async def playmode_ans(client, CallbackQuery, _):
    ...
    # Reconstruct buttons after any change
    playmode = await get_playmode(chat_id)
    is_non_admin = await is_nonadmin_chat(chat_id)
    playty = await get_playtype(chat_id)
